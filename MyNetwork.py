...
from MyCost import *
from MyActive import *


class Layer():
    def __init__(self, model, acti_func, d_acti_func, input_dim=None, output_dim=None, first_layer=False,
                 last_layer=False, learning_rate=0.0075):
        self.model = model
    ...
    def random_initialize(self):
        self.W = np.random.randn(self.output_dim, self.input_dim) / np.sqrt(
            self.input_dim)  # *np.sqrt(2/self.input_dim)
...
class Model():
    ...
    def create_architecture(self, description, input_size):
        ...
        for index, descr in enumerate(description):
            ...
            layer = Layer(self, activ, d_activ, input_dim, output_dim,
                          first_layer=(index == 0), last_layer=(index == len(description) - 1),
    ...
    def train(self, epocs):
        ...
        for i in range(epocs):
            ...
            if i % 1 == 0:
                print("iteration:cost %i: %f" % (i, cost))
            self.backward_pass()
